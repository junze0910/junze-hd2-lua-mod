...
--
--  表 MountComponentData：djb2 类型哈希 0x3845B1E0
--  记录 120 字节，+0 是 u64 Path（该挂载点使用的武器/模型资源）：
--      drone_mg（实体 0xA0FF2F9A0CA6992A）的记录下标 97，原 +0 =
--        0xA32621E3BDE13379  content/fac_helldivers/equipment/backpacks/drone_mg/drone_mg_weapon
--      换成
--        0x587878FB76F4B9B1  SEAF MG-43
--
--  v6 定位（零布局常量）：
--      锚点 = 12 字节：旧路径(8) + 记录 +8 的常量 0x53BEC437(4)
--      实测全表唯一（新版 324 索引 / 163 记录 × 120 里只命中 1 处）
--      → 不依赖 size / 索引条数 / 记录下标（记录下标 97 会随插入而漂移）
--      幂等：搜"新路径 + 同一常量"的形态即认为已生效。
...
local function flush_log()
  ...
  pcall(function()
    ...
    local file = loader and loader.open_log and loader.open_log('GuardDogMg43.log')
...
if ENV.api and ENV.api < 1 then
  ...
  dump('GuardDogMg43_STATUS.log', msg .. '\n')
  report(msg, true)
  return
end
...
-- ---------------------------------------------------------------- 常量
-- 稳定件：LDLD / version / typeHash；记录内相对偏移；路径哈希本身。
-- 不稳定件（不当指纹）：size（旧 24592 → 新 24744）、索引条数、记录下标。
local MAGIC         = 'LDLD'
...
local CTX           = hex_le('53BEC437')        -- 记录 +8 的常量（上下文校验）
local OLD_PATH      = hex_le('A32621E3BDE13379')-- drone_mg_weapon（原装挂载）
local NEW_PATH      = hex_le('587878FB76F4B9B1')-- SEAF MG-43
local ANCHOR_OLD    = OLD_PATH .. CTX           -- 12 字节锚点，全表唯一
local ANCHOR_NEW    = NEW_PATH .. CTX
local MID_OLD       = OLD_PATH                  -- 复查用（记录 +0..+7）
local MID_NEW       = NEW_PATH
local SELF_PATTERNS = { SIGNATURE, ANCHOR_OLD, ANCHOR_NEW, OLD_PATH, NEW_PATH }
...
local function apply(magic)
  ...
  local function pass(anchor, is_new)
    ...
    while true do
      ...
      if is_new then
        ...
        if write_bytes(address, NEW_PATH) then
          state.slots[address] = true
  ...
  if patched > 0 or already > 0 then
    ...
    report(('已替换：表 0x%X size=%d drone_mg 挂载 %s -> %s（本轮写 %d 处，已是目标 %d 处）'):format(
      magic, size, hex(OLD_PATH), hex(NEW_PATH), patched, already), true)
  end
...
report(('已加载 v6（挂载：内容锚点定位）；前置 loader v%s / API %s（来源 %s）'):format(
  tostring(ENV.version or '?'), tostring(ENV.api or '?'), tostring(ENV.source)))
