-- HD2-Addon: mods/dsh/balanced_exosuit_patriot

-- ===========================================================================
--  更均衡的爱国者/解放者外骨骼 · 携带爱国者版（外骨骼三件套 v1.1）
--
...
--  ③ StratagemSettings · 爱国者外骨骼（StratagemType_PatriotExosuit）的"战备附加"槽
--       additional_stratagem 0 → 10（= StratagemType_EmancipatorExosuit）：
--       **携带爱国者外骨骼战备时，额外携带解放者外骨骼战备**
--  ④ 两台外骨骼通用调整：可用次数（use）3 → 1；冷却（cooldown_duration_success）420.0 → 0.0
--
--  ⚠⚠ 与同系列的另一个版本（携带解放者版）**二选一**：两条记录同时被写成非 0 会形成"战备套娃"（互相附加）
...
--   · 战备：**直接搜 package 值**（2482778796672462694 = packages/generated/loadout/combat_walker），
--     命中点 = 记录内 package 字段；再校验 +8 处的 icon（4138467624065961495）确认布局；
...
local MOD = 'mods/dsh/balanced_exosuit_patriot'
...
-- ---------------------------------------------------------------- 目标（常量）
local ATTACH_VALUE   = 10        -- ★ 战备附加写入值（本版本 = StratagemType_EmancipatorExosuit）
...
local WALKER_PAYLOAD = hex_le('79E4B3D2DA5E45E3')   -- 载具实体 8783342891467425251（同 RACK_B_ENTITY 的字节序）
local WALKER_PACKAGE = hex_le('22749A294788AF66')   -- 2482778796672462694 packages/generated/loadout/combat_walker
...
local ICON_WALKER     = hex_le('396ECA60A6E80E17')   -- 4138467624065961495 icon（对向版用 COUNTER_*）
...
local COUNTER_PACKAGE = hex_le('E72D3E9B05C3DB0B')   -- 16658039432250907403 packages/generated/loadout/combat_walker_obsidian
local COUNTER_ICON    = hex_le('CCA47E13FC4682E8')   -- 14746049704219804392 icon
...
local function flush_log()
  ...
  dump('BalancedExosuitPatriot.log', table.concat(loghist, '\n') .. '\n')
end
...
local function apply_rack(magic)
  ...
  local function do_slot(entity_le, slot, expect_item, node_le, want_item, tag)
    ...
    if write_bytes(address, want_item) then
      ...
      dump('BalancedExosuitPatriot_Addresses.log', table.concat({
    ...
    if icon ~= ICON_WALKER then
      ...
      elseif pre ~= ZERO4 or post ~= ZERO4 then
        ...
        elseif write_bytes(address, want) then
          ...
          dump('BalancedExosuitPatriot_StratSites.log', table.concat({
            ('package 字段地址    = 0x%X（= packages/generated/loadout/combat_walker）'):format(c),
...
local function write_status()
  ...
  dump('BalancedExosuitPatriot_STATUS.log', table.concat({
    first,
    'revision=balanced-exosuit-patriot-1.1' .. (DRY_RUN and '-dryrun' or ''),
    ...
    '改动 = ①EXO-49 槽1 右臂加农炮→加特林炮塔 ②EXO-45 槽0 导弹发射器→左臂加农炮 ③携带爱国者外骨骼战备时额外携带解放者外骨骼战备（战备附加 0→10） ④两台外骨骼 可用次数→1 冷却→0',
...
if ENV.api and ENV.api < 1 then
  ...
  dump('BalancedExosuitPatriot_STATUS.log', msg .. '\n'); report(msg, true); return
end
...
report(('已加载 v1.1·携带爱国者版（更均衡的爱国者/解放者外骨骼：EXO-49 槽1→加特林炮塔 / EXO-45 槽0→左臂加农炮 / 携带爱国者外骨骼战备时额外携带解放者外骨骼战备（%d 号） / 两台外骨骼 可用次数→1、冷却→0）；前置 loader %s / API %s（来源 %s）'):format(
